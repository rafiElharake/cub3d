...
static int parse_texture_line(char *line, t_game *game, t_parse *parse)
{
	char *path;
	char **target;
	int *has_flag;
...
static int parse_color_line(char *line, t_game *game, t_parse *parse)
{
	int r, g, b;
	int *target_color;
	int *has_flag;
...
int parse_file(const char *filename, t_game *game)
{
	...
	while ((line = get_next_line(fd)))
	{
		...
		{
			result = parse_texture_line(line, game, &game->parse_state);
			if (result == 0)
			{
				free(line);
				close(fd);
			...
			if (result == -1)
			{
				result = parse_color_line(line, game, &game->parse_state);
				if (result == 0)
				{
					free(line);
					close(fd);
